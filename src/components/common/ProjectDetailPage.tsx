...
export default function ProjectDetailPage<T extends DetailProject>({
  data,
  basePath,
  copy,
}: {
  ...
  if (!project) {
    return (
      <section className="container-max py-20 text-center">
        <h2 className="mb-6 text-3xl font-bold text-white">{copy.notFound}</h2>
        <Link to={basePath} className="text-white hover:underline">{copy.backLink}</Link>
      </section>
    );
  }
